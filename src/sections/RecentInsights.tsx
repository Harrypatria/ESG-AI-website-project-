...
export function RecentInsights() {
  ...
  const handleShare = (e: React.MouseEvent, platform: string, title: string) => {
    e.preventDefault();
    e.stopPropagation();
            ...
            <AnimatePresence mode="wait">
              {currentInsights.map((post, index) => (
                ...
                >
                  <Link 
                    to={`/insights/${post.id}`}
                    className="p-10 rounded-[2.5rem] bg-white shadow-[12px_12px_24px_#d1d9e6,-12px_-12px_24px_#ffffff] hover:shadow-[16px_16px_32px_#d1d9e6,-16px_-16px_32px_#ffffff] transition-all group flex flex-col border border-transparent hover:border-brand-primary/10 h-full text-left relative"
                  >
                    ...
                    <div className="mt-10 pt-8 border-t border-brand-border flex items-center justify-between">
                      <div className="flex items-center gap-2 text-brand-text font-medium text-[10px] tracking-[0.3em] group/btn uppercase hover:text-brand-primary transition-colors">
                        Read more 
                      ...
                      <div className="flex items-center gap-3">
                        <button 
                          onClick={(e) => handleShare(e, "twitter", post.title)}
                        ...
                        <button 
                          onClick={(e) => handleShare(e, "linkedin", post.title)}
                          className="p-2 rounded-full hover:bg-brand-surface text-brand-muted hover:text-brand-primary transition-all"
                        ...
                        <button 
                          onClick={(e) => handleShare(e, "threads", post.title)}
                          className="p-2 rounded-full hover:bg-brand-surface text-brand-muted hover:text-brand-primary transition-all"
